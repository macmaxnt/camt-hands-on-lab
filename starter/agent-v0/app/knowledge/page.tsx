...
export default function KnowledgePage() {
  ...
  return <main className="lab">
    <header className="hero"><div><p className="eyebrow">{text.eyebrow}</p><h1>{text.title}</h1><p className="subtitle">{text.subtitle}</p></div><div className="header-actions"><nav><a href="/">{text.chat}</a><a href="/security">{text.security}</a><a href="/chat" className="chat-nav-link">Evidence console</a><a href="/agent">{text.agent}</a></nav><SessionControl labels={text.session} onRoleChange={() => { setSelected(null); setForbidden(false); void loadDocuments(); }} /><div className="language" aria-label="Language"><button className={language === "en" ? "selected" : ""} onClick={() => setLanguage("en")}>EN</button><button className={language === "th" ? "selected" : ""} onClick={() => setLanguage("th")}>ไทย</button></div></div></header>
