...
const copy = {
  en: {
    ...
    empty: "Ask a question to retrieve the seeded corpus.", error: "Could not generate an answer.", docs: "accessible documents", ledger: "Knowledge base", security: "Lab 2 security backlog", agent: "Agent backlog", scope: "Scope: local synthetic corpus · only authorized excerpts go to the configured NVIDIA Build API.", session: { access: "Access", public: "Public", staff: "Staff", enable: "Enable staff", pin: "Staff PIN", cancel: "Cancel", logout: "Log out", submitting: "Checking…", unavailable: "Staff access is unavailable." },
  },
  th: {
    ...
    empty: "ถามคำถามเพื่อค้นใน corpus ที่เตรียมไว้.", error: "ไม่สามารถสร้างคำตอบได้", docs: "เอกสารที่เข้าถึงได้", ledger: "คลังความรู้", security: "รายการความปลอดภัย Lab 2", agent: "งานค้างเอเจนต์", scope: "ขอบเขต: corpus สังเคราะห์ในเครื่อง · ส่งเฉพาะ excerpt ที่ได้รับสิทธิ์ไปยัง NVIDIA Build.", session: { access: "สิทธิ์", public: "สาธารณะ", staff: "เจ้าหน้าที่", enable: "เปิดใช้ staff", pin: "Staff PIN", cancel: "ยกเลิก", logout: "ออกจากระบบ", submitting: "กำลังตรวจสอบ…", unavailable: "ไม่สามารถเปิดใช้ staff ได้" },
  },
} as const;
...
export default function Home() {
  ...
  return <main className="lab">
    <header className="hero"><div><p className="eyebrow">{text.eyebrow}</p><h1>{text.title}</h1><p className="subtitle">{text.subtitle}</p></div><div className="header-actions"><nav><a href="/knowledge">{text.ledger}</a><a href="/security">{text.security}</a><a href="/chat" className="chat-nav-link">Evidence console</a><a href="/agent">{text.agent}</a></nav><SessionControl labels={text.session} onRoleChange={() => void refreshCorpus()} /><div className="language" aria-label="Language"><button className={language === "en" ? "selected" : ""} onClick={() => setLanguage("en")}>EN</button><button className={language === "th" ? "selected" : ""} onClick={() => setLanguage("th")}>ไทย</button></div></div></header>
