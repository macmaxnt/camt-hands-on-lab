...
const copy = {
  en: {
    eyebrow: "AIAT × CAMT · Lab 2", title: "Security workbench", subtitle: "Blue-team controls protect every answer path.", chat: "RAG chat", knowledge: "Knowledge base", agent: "Agent backlog", warning: "Audit records contain fingerprints and decisions only; they never retain raw prompts or answers.", implemented: "Implemented controls", audit: "Recent security audit", staffOnly: "Enable staff access to view privacy-minimised audit events.", emptyAudit: "No audit events yet.",
...
export default function SecurityPage() {
  ...
  return <main className="lab"><header className="hero"><div><p className="eyebrow">{text.eyebrow}</p><h1>{text.title}</h1><p className="subtitle">{text.subtitle}</p></div><div className="header-actions"><nav><a href="/">{text.chat}</a><a href="/knowledge">{text.knowledge}</a><a href="/chat" className="chat-nav-link">Evidence console</a><a href="/agent">{text.agent}</a></nav><SessionControl labels={text.session} onRoleChange={() => void loadAudit()} /><div className="language" aria-label="Language"><button className={language === "en" ? "selected" : ""} onClick={() => setLanguage("en")}>EN</button><button className={language === "th" ? "selected" : ""} onClick={() => setLanguage("th")}>ไทย</button></div></div></header><p className="warning" role="alert">{text.warning}</p>
