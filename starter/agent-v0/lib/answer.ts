import type { GuardrailDecision, SafeAnswerStatus, UserRole } from "./contracts";
...
export interface FinalAnswer { status: SafeAnswerStatus; answer: string; citations: Citation[]; }
...
export async function* streamAnswer(
  ...
  const repository = options.securityRepository ?? getSeedCorpusService();
  ...
  const finish = async function* (final: FinalAnswer, decision: GuardrailDecision, reasons: string[], count: number, incident = false): AsyncGenerator<AnswerStreamEvent> {
    repository.recordAudit(newAuditRecord(secret, {
  ...
  for (const chunk of retrieval.chunks) {
    ...
    repository.quarantineChunk({ chunkId: chunk.id, reasonCode: match.reason, detectorVersion: match.detectorVersion, contentFingerprint: fingerprint(secret, "chunk", chunk.text) });
...
function validateProviderAnswer(raw: string, authorized: Array<{ fingerprint: string; citation: Citation }>, language: Language): FinalAnswer {
  ...
  if (value.status === "not_found") return { status: "not_found", answer: noEvidenceMessage(language), citations: [] };
  if (value.status === "ambiguous") return { status: "ambiguous", answer: value.answer.trim(), citations: [] };
  return { status: "grounded", answer: value.answer.trim(), citations: citations as Citation[] };
}
