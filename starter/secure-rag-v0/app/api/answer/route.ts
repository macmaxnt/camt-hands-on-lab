...
export async function POST(request: Request) {
  ...
  try { input = validateInput(await request.json() as Record<string, unknown>); }
...
function validateInput(body: Record<string, unknown>): AnswerInput {
  ...
  // TODO(Lab 2): resolve an HttpOnly session, authorize the request, and run input/PII policy checks here.
  return { question, mode: body.mode as RetrievalMode, language: body.language as Language };
}
