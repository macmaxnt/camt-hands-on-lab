...
test("streams a cited NVIDIA-compatible answer and removes invalid numbered citations", async () => {
  let request: Request | undefined;
  const upstream = `data: ${JSON.stringify({ choices: [{ delta: { content: "STATUS: grounded\nLearner data must be protected [1]. [9]" } }] })}\n\ndata: [DONE]\n\n`;
  const events = [] as Array<{ event: string; data: unknown }>;
  for await (const event of streamAnswer({ question: "How should data be handled?", mode: "semantic", language: "en" }, {
    config: { baseUrl: "https://integrate.api.nvidia.com/v1", apiKey: "test-key", model: "nvidia/test" },
    fetchImpl: async (input, init) => { request = new Request(input, init); return new Response(upstream); },
  ...
  const payload = await request?.json() as { model: string; stream: boolean; messages: unknown[] };
  ...
  assert.equal(final.answer, "Learner data must be protected [1].");
...
test("does not invoke NVIDIA Build when retrieval returns no evidence", async () => {
  let called = false;
  const events = [] as Array<{ event: string; data: unknown }>;
  for await (const event of streamAnswer({ question: "Unknown topic", mode: "keyword", language: "en" }, {
